document.addEventListener('DOMContentLoaded', () => {
  ...
  const searchForm = document.getElementById('search__form') as HTMLFormElement;
  const searchInput = document.getElementById('search__input') as HTMLInputElement;
  const searchSubmit = document.getElementById('search__submit') as HTMLButtonElement;
  ...
  searchInput.addEventListener('input', event => {
    ...
    searchValue = (event.target as HTMLInputElement).value; // Обновляем значение поиска
  });
