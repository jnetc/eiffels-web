document.addEventListener('DOMContentLoaded', async () => {
  ...
  const openDialogAUP = document.getElementById('open__aup-dialog') as HTMLButtonElement; // Button to open the dialog
  const dialogAUP = document.getElementById('aup-dialog') as HTMLDialogElement; // The dialog itself
  const formAUP = document.getElementById('aup__form') as HTMLFormElement; // Form inside the dialog
    ...
    document.addEventListener('change', (event: Event) => {
      ...
      if (file) {
        ...
        reader.onload = (e: ProgressEvent<FileReader>) => {
          ...
          if (e.target?.result) {
            imageUrl.src = e.target.result as string; // Set the image source for preview
          }
        };
  ...
  formAUP.addEventListener('submit', (event: SubmitEvent) => {
    ...
    const buttonName = button.querySelector('.btn-text') as HTMLSpanElement;
